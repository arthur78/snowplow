...
object PiiConstants {
   ...
   * it preserves type safety, and it can be easily replaced in teh future by generated code that will use the config as
   * input.
   */
  ...
  override def applyStrategy(fieldValue: String): String =
    if (fieldValue != null)
      compact(render(parse(fieldValue).transformField {
        case JField("data", JArray(contexts)) =>
          ("data", JArray(contexts.map {
            case JObject(context) => modifyObjectIfSchemaMatches(context)
            case x                => x
          }))
        case JField("data", JObject(unstructEvent)) =>
          ("data", modifyObjectIfSchemaMatches(unstructEvent))
      }))
    else null

  private def modifyObjectIfSchemaMatches(context: List[(String, json4s.JValue)]): JObject = {
    val fields: Map[String, JValue] = context.toMap.filterKeys(s => s == "schema" || s == "data")
    fields
      .get("schema")
      .flatMap(s =>
